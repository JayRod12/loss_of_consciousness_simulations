# Experiment 12, PING modular connectome with a driving thalamus
# Stems from experiment 10 full PING connectome
...
#    recorded_neurons = [0, 1, 50, 90]
    ...
    M_THEX = SpikeMonitor(THEX_G)
    M_THIN = SpikeMonitor(THIN_G)
#    M_V = StateMonitor(EX_G, 'v', record=recorded_neurons)
    ...
    X3, Y3 = np.array(M_THEX.t/ms), np.array(M_THEX.i)
    X4, Y4 = np.array(M_THIN.t/ms), np.array(M_THIN.i)
